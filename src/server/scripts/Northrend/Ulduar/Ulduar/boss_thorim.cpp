...
public:
    ...
    struct boss_thorimAI : public ScriptedAI
    {
        ...
        {
            if (who && _isHitAllowed && who->GetPositionZ() > 430 && (who->IsPlayer() || who->IsNPCBot()))
            {
                _isHitAllowed = false;
                DisableThorim(false);
        ...
        {
            if (spellInfo->Id == SPELL_LIGHTNING_CHARGE_DAMAGE && (target->IsPlayer() || target->IsNPCBot()))
...
public:
    ...
    struct boss_thorim_start_npcsAI : public ScriptedAI
    {
        ...
        {
            if (!_playerAttack && who && ((who->IsPlayer() || who->IsNPCBot()) || who->GetOwnerGUID().IsPlayer()))
            {
                if (me->GetInstanceScript())
